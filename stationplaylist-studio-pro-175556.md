...
Voice track / song intro overlapping.
...
96 cart slots for instant jingles/sound effects etc.
24 personalised carts per user/announcer login.
...
Support for automatic time-of-day announcements and temperature announcements (audio files separate).
Auto-start in automated mode after power failure.
Detailed logging for royalty payments and advertisement billing.
Single keystrokes for Start, Stop, Mic and more.
Fail safe redundancy using Studio Monitor.
Integration with external devices using LPT port (consoles/switches).
